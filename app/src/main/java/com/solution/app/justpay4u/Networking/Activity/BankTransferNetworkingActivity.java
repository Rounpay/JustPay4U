...
import com.solution.app.justpay4u.Networking.Adapter.BankAccountNetworkingAdapter;
...
public class BankTransferNetworkingActivity extends AppCompatActivity {
    ...
    RecyclerView recycler_view, balanceRecyclerView;
    View noNetworkView;
    View addBankView, dmtReportView, listView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Handler(Looper.getMainLooper()).post(() -> {
            ...
            balanceResponse = getIntent().getParcelableExtra("BalanceData");
            fromWalletId = getIntent().getIntExtra("FromWalletId", 0);
            fromWalletName = getIntent().getStringExtra("FromWallet");
            opTypeId = getIntent().getIntExtra("OpTypeId", 0);
            walletBal = getIntent().getStringExtra("OnlyBalanceData");
            fromWalletAmt = getIntent().getStringExtra("amount");
            ...
            findViews();
            amonut.setText(fromWalletAmt);
            balanceName.setText(fromWalletName);
            /*showBalanceData();*/
            HitApi();
        });

//        amonut.setText(balanceResponse.getBalanceData().get(0).getBalance());

//        Log.d("dfkglfjn", String.valueOf(balanceResponse.getBalanceData().get(0).getBalance()));

    }

    void findViews() {
        balanceName = findViewById(R.id.name);
        amonut = findViewById(R.id.amount);
        ...
        mAdapter = new BankAccountNetworkingAdapter(isSattlemntAccountVerify, mBankListObjects, this);
        recycler_view.setAdapter(mAdapter);
        noDataView = findViewById(R.id.noDataView);
        addBankView = findViewById(R.id.addBankView);
        dmtReportView = findViewById(R.id.dmtReportView);
        noNetworkView = findViewById(R.id.noNetworkView);
        TextView errorMsg = findViewById(R.id.errorMsg);
        errorMsg.setText("Bank Account is not available, click on Add button to add new account");
       ...
       /* BankListResponse bankListResponse = getIntent().getParcelableExtra("BankList");
        ...
        addBankView.setOnClickListener(v -> {
            startActivityForResult(new Intent(this, BankUpdateAddNetworkingActivity.class)
                    .setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP), INTENT_UPDATE);
        });
        dmtReportView.setOnClickListener(v -> {
            Intent m = new Intent(this, DMRReportActivity.class);
  ...
  /*  private void showBalanceData() {
        ...
        if (balanceResponse != null && balanceResponse.getBalanceData() != null && balanceResponse.getBalanceData().size() > 0) {
            WalletBalanceNetworkingAdapter mAdapter = new WalletBalanceNetworkingAdapter(BankTransferNetworkingActivity.this,
                    balanceResponse.getBalanceData(), R.layout.adapter_wallet_balance_with_icon, fromWalletId);


            balanceRecyclerView.setAdapter(mAdapter);
        } else {
            balanceResponse = ApiFintechUtilMethods.INSTANCE.getBalanceResponse(mAppPreferences);
            if (balanceResponse != null && balanceResponse.getBalanceData() != null
                    && balanceResponse.getBalanceData().size() > 0) {
                ...
                ApiFintechUtilMethods.INSTANCE.Balancecheck(this, null, mLoginDataResponse,
                        ...
                        mAppPreferences, null, object -> {
                            balanceResponse = (BalanceResponse) object;
                            if (balanceResponse != null && balanceResponse.getBalanceData() != null
                                    && balanceResponse.getBalanceData().size() > 0) {

                                for(BalanceData item : balanceResponse.getBalanceData()){
                                    if(item.getId()==fromWalletId){

                                        amonut.setText(item.getWalletCurrencySymbol()+" "+ Utility.INSTANCE.formatedAmountWithOutRupees(item.getBalance() + ""));

                                    }
                                }
                                                        *//*WalletBalanceAdapter mAdapter = new WalletBalanceAdapter(WalletToWalletTransferNetworkingActivity.this,
                                                                balanceResponse.getBalanceData(), R.layout.adapter_wallet_balance_form);
            ...
            call.enqueue(new Callback<BasicResponse>() {
                                ...
                                if (data.getStatuscode() == 1) {
                                    if (data.getData() != null) {
                                        if (data.getData().getStatuscode() == 1) {
                                            ApiFintechUtilMethods.INSTANCE.Successful(BankTransferNetworkingActivity.this, data.getData().getMsg() + "");

                                            HitApi();
                                        } else {
                                            ApiFintechUtilMethods.INSTANCE.Error(BankTransferNetworkingActivity.this, data.getData().getMsg() + "");

                                        }
                                    } else {
                                        ApiFintechUtilMethods.INSTANCE.Successful(BankTransferNetworkingActivity.this, data.getMsg() + "");
                                    ...
                                    //setResult(RESULT_OK);
                                    ApiFintechUtilMethods.INSTANCE.Error(BankTransferNetworkingActivity.this, data.getMsg() + "");
                                }

                            } else {
                                ApiFintechUtilMethods.INSTANCE.Error(BankTransferNetworkingActivity.this, getString(R.string.some_thing_error));
                            }
                ...
                public void onFailure(Call<BasicResponse> call, Throwable t) {
                    ...
                    try {
                        ...
                        ApiFintechUtilMethods.INSTANCE.apiFailureError(BankTransferNetworkingActivity.this, t);
                    } catch (IllegalStateException ise) {
                        if (loader != null && loader.isShowing()) {
                            loader.dismiss();
                        }
                        ...
                        if (response.isSuccessful()) {
                            ...
                            if (data != null) {
                                if (data.getStatuscode() == 1) {
                                    if (data.getData() != null) {
                                        if (data.getData().getStatuscode() == 1) {
                                            ApiFintechUtilMethods.INSTANCE.Processing(BankTransferNetworkingActivity.this, data.getData().getMsg() + "");
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == INTENT_UPDATE && resultCode == RESULT_OK) {
            setResult(RESULT_OK);
            HitApi();
        } else if (requestCode == INTENT_SEND_MONEY && resultCode == RESULT_OK) {
            ApiFintechUtilMethods.INSTANCE.Balancecheck(this, null, mLoginDataResponse,
                    deviceId, deviceSerialNum,
                    mAppPreferences, null, object -> {
                        balanceResponse = (BalanceResponse) object;
   ...
   /* @Override
    ...
    public void sendMoney(SettlementAccountData operator) {

        Intent transferIntent = new Intent(this, MoneyTransferNetworkingActivity.class);
        transferIntent.putExtra("AccountData", operator);
        transferIntent.putExtra("WalletId", fromWalletId);
        transferIntent.putExtra("OpTypeId", opTypeId);
