...
public class DMRActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Handler(Looper.myLooper()).post(() -> {
            ...
            loader = new CustomLoader(DMRActivity.this, android.R.style.Theme_Translucent_NoTitleBar);
    ...
    private void findViews() {
        ...
        senderNumberEt.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length() == 10) {
                    loader.show();
                    GetSender(DMRActivity.this, senderNumberEt.getText().toString(), loader,
                            mLoginDataResponse);
                }
            }
        });
    ...
    void setDbData() {
        ...
        String senderName = mAppPrefrences.getString(ApplicationConstant.INSTANCE.senderNamePref);
        String senderLimit = mAppPrefrences.getString(ApplicationConstant.INSTANCE.senderBalance);
        if (senderNum != null && senderNum.length() > 0 &&
                senderName != null && senderName.length() > 0 &&
                senderLimit != null && senderLimit.length() > 0) {
            GetSender(senderNum, senderName, senderLimit);
    ...
    public void GetSender(String mobileNo, String senderName, String senderBalance) {
        ...
        addressEt.setText("");
        dobTv.setText("");
        setCurrentDetail(mobileNo, senderName, senderBalance);
    }
    ...
    public void setCurrentDetail(String number, String name, String balance) {
        ...
        remaining.setText(Utility.INSTANCE.formatedAmountWithOutRupees(balance + ""));
        dmrLoginLayout.setVisibility(View.GONE);
        senderLayout.setVisibility(View.VISIBLE);

        RecentDmrLogin mRecentDmrLogin = new RecentDmrLogin(name.trim(), number.trim(), balance.trim());
    ...
    public void onClick(View v) {
        ...
        if (v == dobView) {
            ...
            DatePickerDialog mDatePicker = new DatePickerDialog(DMRActivity.this, date, myCalendar
       ...
       /* if (v == phoneBookIv) {
        ...
        if (v == dmrLogout) {
            ...
            mCustomAlertDialogs.WarningWithCallBack("Are you sure, you want to logout this sender?", "Logout", true, new CustomAlertDialog.DialogCallBack() {
                @Override
        ...
        if (v == addBeneficiaryView) {
            ...
            Intent i = new Intent(DMRActivity.this, AddBeneficiaryActivity.class);
        ...
        if (v == dmtReportView) {
            ...
            Intent m = new Intent(DMRActivity.this, DMRReportActivity.class);
    ...
    void createAccount() {
        ...
        if (ApiFintechUtilMethods.INSTANCE.isNetworkAvialable(DMRActivity.this)) {
            loader.show();
            loader.setCancelable(false);
            loader.setCanceledOnTouchOutside(false);

            CreateSender(DMRActivity.this,
            ...
            ApiFintechUtilMethods.INSTANCE.NetworkError(DMRActivity.this);
        }
    }
    ...
    public void onActivityResult(int reqCode, int resultCode, Intent data) {
        ...
        /*else if (reqCode == PICK_CONTACT && resultCode == RESULT_OK) {
            ...
            Cursor c = DMRActivity.this.getContentResolver().query(contactData, null, null, null, null);
            if (c.moveToFirst()) {
                ...
                if (hasPhone.equalsIgnoreCase("1")) {
                    Cursor phones = DMRActivity.this.getContentResolver().query(
...
//                            Log.e("number is:",Number);
                ...
                if (!Number.equals("")) {
                    ...
                    Toast.makeText(DMRActivity.this, "Please select a valid number", Toast.LENGTH_SHORT).show();
                }
            }
        }
    ...
    public void CreateSender(final Activity context, final String MobileNumber, String name, String lastName,
        ...
        try {
            ...
            Call<RechargeReportResponse> call = git.CreateSender(new GetSenderRequest(mSenderobject, LoginDataResponse.getData().getUserID() + "",
                ...
                public void onResponse(Call<RechargeReportResponse> call, retrofit2.Response<RechargeReportResponse> response) {
                    try {
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null) {
                                if (response.body().getStatuscode() == 1) {
                                    if (mDialog != null && mDialog.isShowing()) {
                                        ApiFintechUtilMethods.INSTANCE.Successful(DMRActivity.this, "OTP has been resend successfully");
                                        if (timerTv != null && resendCodeTv != null) {
                                            ApiFintechUtilMethods.INSTANCE.setTimer(timerTv, resendCodeTv);
                                        }
                                    } else {
                                        ApiFintechUtilMethods.INSTANCE.openOtpDialog(DMRActivity.this, 6, MobileNumber, new ApiFintechUtilMethods.DialogOTPCallBack() {
                                            @Override
                                            public void onPositiveClick(String otpValue, TextView timerTv, TextView resendCodeTv, Dialog mDialog) {
                                                loader.show();
                                                VerifySender(DMRActivity.this, MobileNumber, otpValue, response.body().getSid(), loader, mLoginDataResponse, mDialog);
                                            }
                            ...
                            ApiFintechUtilMethods.INSTANCE.apiErrorHandle(DMRActivity.this, response.code(), response.message());
                        }
                    } catch (Exception e) {
                        if (loader != null && loader.isShowing()) {
                            loader.dismiss();
    ...
    public void VerifySender(final Activity context, final String MobileNumber, String otp, String sid, final CustomLoader loader,
        ...
        try {
            FintechEndPointInterface git = ApiClient.getClient().create(FintechEndPointInterface.class);
            Call<RechargeReportResponse> call = git.VerifySender(new GetSenderRequest(new SenderObject(MobileNumber, otp, sid),
                        ...
                        if (response.isSuccessful()) {
                            if (response.body() != null) {
                                if (response.body().getStatuscode() == 1) {
                                    ...
                                    loader.show();
                                    GetSender(DMRActivity.this, senderNumberEt.getText().toString(), loader, mLoginDataResponse);
                                } else {
                                    if (!response.body().getIsVersionValid()) {
                                        ApiFintechUtilMethods.INSTANCE.versionDialog(context);
                                    } else {
                            ...
                            if (response.body() != null) {
                                if (response.body().getStatuscode() == 1) {
                                    if (!response.body().isSenderNotExists()) {
                                        ApiFintechUtilMethods.INSTANCE.setSenderNumber(MobileNumber, response.body().getSenderName(),
                                                response.body().getSenderBalance(), response.body().getRemainingLimit() + "", response.body().getAvailbleLimit() + "", gson.toJson(response.body()), mAppPrefrences);
                                        senderNumberStr = MobileNumber + "";
                                        GetSender(MobileNumber + "", response.body().getSenderName() + "",
                                                response.body().getSenderBalance() + "");
    ...
    private void GetBeneficiary() {
        ...
        ApiFintechUtilMethods.INSTANCE.GetBeneficiary(DMRActivity.this, senderNumberStr, mLoginDataResponse, loader, mAppPrefrences,
    ...
    public void confirmationDialog(BenisObject operator) {
        ...
        view.findViewById(R.id.ok).setOnClickListener(v -> {
            ...
            if (ApiFintechUtilMethods.INSTANCE.isNetworkAvialable(this)) {
                ...
                loader.setCancelable(false);
                loader.setCanceledOnTouchOutside(false);
                ApiFintechUtilMethods.INSTANCE.Deletebeneficiary(this, senderNumberStr, operator.getBeneID(), loader, mLoginDataResponse,
    ...
    public void sendMoneyClick(BenisObject operator) {
        Intent transferIntent = new Intent(this, MoneyTransferActivity.class);
