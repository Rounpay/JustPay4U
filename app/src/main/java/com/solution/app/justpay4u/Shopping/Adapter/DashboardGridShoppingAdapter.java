...
public class DashboardGridShoppingAdapter extends RecyclerView.Adapter<DashboardGridShoppingAdapter.MyViewHolder> {
    ...
    public DashboardGridShoppingAdapter(Activity mContext, List<DashboardProductListData> transactionsList, int layout) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
       ...
       /* if (viewType == LIST_ITEM) {
            itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.grid_item_layout, null);
        } else {
            itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_layout, null);
        }*/
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        final DashboardProductListData operator = transactionsList.get(position);
           ...
           /* if (operator.getWishList().equalsIgnoreCase("1")) {
            ...
            if (operator.getDiscount() == 0 || operator.getDiscount() == 0.00) {
                ...
                if (operator.isDiscountType()) {
                    ...
                    holder.discount_label.setText(ApiShoppingUtilMethods.INSTANCE.formatedAmountWithRupees((operator.getDiscount() /** operator.getQuantity()*/) + "") + " Off");
                }
                /*holder.discount_label.setText("\u20B9 " + ApiShoppingUtilMethods.INSTANCE.formatedAmount(operator.getDiscount() + "") + " Off");*/
            }
            ...
            /*if (isLogin == 1) {
                ...
                holder.moreIcon.setOnClickListener(v -> showPopupDialog(v, operator));
            } else {*/
    ...
    public void usingCustomIcons(MyViewHolder holder) {
        ...
        holder.vectorAddToCard.setUnlikeDrawable(new BitmapDrawable(mContext.getResources(),
                new IconicsDrawable(mContext, CommunityMaterial.Icon.cmd_cart).colorRes(R.color.greycon).sizeDp(40).toBitmap()));

        //shown when the button is liked!
        holder.vectorAddToCard.setLikeDrawable(new BitmapDrawable(mContext.getResources(),
                new IconicsDrawable(mContext, CommunityMaterial.Icon.cmd_cart).colorRes(R.color.greycon).sizeDp(40).toBitmap()));

        holder.vectorLike.setUnlikeDrawable(new BitmapDrawable(mContext.getResources(),
                new IconicsDrawable(mContext, CommunityMaterial.Icon.cmd_heart).colorRes(R.color.greycon).sizeDp(40).toBitmap()));

        //shown when the button is liked!
        holder.vectorLike.setLikeDrawable(new BitmapDrawable(mContext.getResources(),
                new IconicsDrawable(mContext, CommunityMaterial.Icon.cmd_heart).colorRes(R.color.red).sizeDp(40).toBitmap()));
    ...
    void saveCartApi(final DashboardProductListData operator, LikeButton vectorAddToCard) {
        ...
        if (ApiShoppingUtilMethods.INSTANCE.isNetworkAvialable(mContext)) {
            ...
            ApiShoppingUtilMethods.INSTANCE.AddCart(mContext, operator.getPosId() + "", "1",
                    new ApiShoppingUtilMethods.ApiHitCallBack() {
                        ...
                        public void onSucess(final Object response) {
                            ...
                            if (data.getStatuscode() == 1) {
                                ...
                                if (mContext instanceof ShoppingDashboardActivity) {
                                    ((ShoppingDashboardActivity) mContext).cartUpdate(mCartItemCount);
                                }
                                if (mContext instanceof ItemDetailsShoppingActivity) {
                                    ((ItemDetailsShoppingActivity) mContext).cartUpdate(mCartItemCount);
                                }
        ...
        popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(MenuItem menuItem) {
                if (menuItem.getItemId() == R.id.share) {
                    shareLink(operator.getProductName() != null && !operator.getProductName().isEmpty() ? operator.getProductName() : operator.getTitle(), operator.getAffiliateShareLink());
                    return true;
...
*/
   /*private void showPopupDialog(View actionView) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.more_popup_dialog, null);


        BubbleDialog dialog = new BubbleDialog(mContext);
        dialog.addContentView;
        dialog.setClickedView(actionView);
       // dialog.calBar(true);
        dialog.setPosition(BubbleDialog.Position.BOTTOM, BubbleDialog.Position.TOP);
        dialog.autoPosition(Auto.UP_AND_DOWN);
        dialog.show();
        dialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
        TextView shareTv = view.findViewById(R.id.share);
        TextView affiliateTv = view.findViewById(R.id.affiliate);
        TextView levelTv = view.findViewById(R.id.level);
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View view) {
           super(view);
            itemView = view;
            tvOriginalPrice =  view.findViewById(R.id.tv_orignal_price);
