...
import com.solution.app.justpay4u.Networking.Adapter.SponserTeamNetworkingAdapter;
...
public class SponserTeamNetworkingActivity extends AppCompatActivity {
    ...
    SponserTeamNetworkingAdapter mAdapter;
    int businessType;
    ArrayList<MemberListData> transactionsObjects = new ArrayList<>();
    EditText search_all;
    CustomLoader loader;

    public View noDataView, noNetworkView, retryBtn, searchViewLayout;
    TextView errorMsg;
    private CustomFilterDialog mCustomFilterDialog;
    private String filterFromDate = "", filterToDate = "", filterStatus = "All";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        new Handler(Looper.getMainLooper()).post(() -> {
            setContentView(R.layout.activity_networking_sponser_team);
    ...
    void findViews() {
        ...
        filterStatus = intent.getStringExtra("Deactive");
        filterLevelNo = intent.getIntExtra("Level", 1);
        downlineID = intent.getStringExtra("DownlineID");

        maxReportDisplayLevel = intent.getIntExtra("maxReportDisplayLevel", 0);
        isMultiLayerTeamDisplay = intent.getBooleanExtra("isMultiLayerTeamDisplay", false);
        businessType = getIntent().getIntExtra("BusinessType", 0);

        recycler_view = findViewById(R.id.recycler_view);
        mAdapter = new SponserTeamNetworkingAdapter(transactionsObjects, maxReportDisplayLevel, isMultiLayerTeamDisplay, businessType, SponserTeamNetworkingActivity.this);
    ...
    private void HitApi(Activity context, boolean IsRecent) {
        if (ApiFintechUtilMethods.INSTANCE.isNetworkAvialable(this)) {
            ...
            try {
                ...
                Call<MemberListResponse> call = git.SponserList(new MemberListRequest(new BasicRequest(loginPrefResponse.getData().getUserID(), loginPrefResponse.getData().getLoginTypeID(), ApplicationConstant.INSTANCE.APP_ID, deviceId, "", BuildConfig.VERSION_NAME, deviceSerialNum, loginPrefResponse.getData().getSessionID(), loginPrefResponse.getData().getSession()), new MemberListRequest(IsRecent, downlineID, filterDateType + "", filterLevelNo, filterFromDate, filterToDate, filterStatus)));
    ...
    public void openFilter() {
        ...
        LinearLayout searchUserIdView = sheetView.findViewById(R.id.searchUserIdView);
        searchUserIdView.setVisibility(View.GONE);
        EditText searchUserIdEt = sheetView.findViewById(R.id.searchUserIdEt);
        searchUserIdEt.setText(loginPrefResponse.getData().getUserID());
...
//        filterStaus
    ...
    /*    LinearLayout statusView = sheetView.findViewById(R.id.statusView);
        ...
        if (filterStatus.equalsIgnoreCase("0") || filterStatus.equalsIgnoreCase("Deactive")) {
            statusChooserTv.setText("Deactive");
        } else if (filterStatus.equalsIgnoreCase("1") || filterStatus.equalsIgnoreCase("Active")) {
...
//                filterStaus = "0";
        ...
        BottomSheetBehavior.from(mBottomSheetDialog.findViewById(R.id.design_bottom_sheet)).setState(BottomSheetBehavior.STATE_EXPANDED);
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (ApiNetworkingUtilMethods.INSTANCE.maxReportDisplay > 1) {
