package com.dreamseller.imagepicker;
...
import com.dreamseller.image.cropper.CropImage;
import com.dreamseller.image.cropper.CropImageView;
...
import static com.dreamseller.image.cropper.CropImage.CAMERA_CAPTURE_PERMISSIONS_REQUEST_CODE;
import static com.dreamseller.image.cropper.CropImage.PICK_IMAGE_CHOOSER_REQUEST_CODE;
import static com.dreamseller.image.cropper.CropImage.getGalleryIntents;
...
public class ImagePicker implements ImagePickerContract {
    ...
    private boolean withCrop, cropWithoutRatio;
    private static boolean includeGallery;
    ...
    @SuppressLint("NewApi")
    @Override
    public void choosePicture(boolean includeCamera, boolean includeGallery) {
        ...
        if (needToAskPermissions()) {
            ...
            startImagePickerActivity(includeCamera, includeGallery);
        }
    }

    @Override
    public void choosePictureWithoutPermission(boolean includeCamera, boolean includeGallery) {
        this.includeGallery = includeGallery;
        startImagePickerActivity(includeCamera, includeGallery);
    }
    ...
    public void openCamera() {
        if (needToAskPermissions()) {
            ...
            cameraIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
    ...
    public void handlePermission(int requestCode, int[] grantResults) {
        ...
        if (requestCode == CAMERA_CAPTURE_PERMISSIONS_REQUEST_CODE_WITH_CAMERA) {
            if (grantResults.length > 0 && grantResults[0] == PERMISSION_GRANTED) {
                startImagePickerActivity(true, includeGallery);
            } else {
        ...
        if (requestCode == CAMERA_CAPTURE_PERMISSIONS_REQUEST_CODE_WITHOUT_CAMERA) {
            if (grantResults.length > 0 && grantResults[0] == PERMISSION_GRANTED) {
                startImagePickerActivity(false, includeGallery);
            } else {
                Toast.makeText(activity, R.string.canceling, Toast.LENGTH_SHORT).show();
            }
    ...
    private Intent getCameraIntent() {
        ...
        String authority = activity.getPackageName() + ".smart-image-picker-provider";
    ...
    private void startImagePickerActivity(boolean includeCamera, boolean includeGallery) {
        ...
        chooserIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
